...
Using static functions to create new instances of the policies provides an easy way to create and use a policy without needing to store the policy instance in a variable. The policy is created and immediately pushed onto the policy queue. The policy is then discarded and will be recreated the next time it is needed.
...
\svfile{example_code/v6_improved_api/policy.svh}{Expanded \code{policy} interface class}
...
The complete policy package is available in Appendix \ref{policy_pkg}, with additional macro examples available in Appendix \ref{policy_macros}. A functional package is also available \cite{b6} which can be included directly in a project to start using policies immediately.
